...
async def handle_tips_code(update: Update, context: CallbackContext) -> None:
    ...
    if message.replace(" ", "").replace("\n", "").isdigit():
        ...
        lista_empresas = {'406','407','410', '412'}
        # empresa = message[:3]
        # print(message[:3])
                    ...
                    if resposta_funcionario:
                        ...
                        try:
                            ...
                            if funcionario_codigo is not None:
                                ...
                                data_atual = datetime.now().strftime("%Y-%m-%d")
                                cursor.execute('SELECT * FROM TLG_BIPNOTIFIC WHERE COD_GRUPO = %s AND HORA_ENVIO >= %s and troca_fun = 2  ORDER BY HORA_ENVIO DESC LIMIT 1',(chat_id,data_atual,))
                                troca_fun = cursor.fetchone()
                                if troca_fun:
                                    cursor.execute('update tlg_Bipnotific set empr = %s, codfun = %s, hora_retorno = %s where cod_grupo = %s and  troca_fun = 2',(cod_empr,cod_fun,agora, chat_id,))
                                    database.commit()
                                    ...
                                    cursor.execute('SELECT * FROM TLG_BIPNOTIFIC WHERE COD_GRUPO = %s AND HORA_ENVIO >= %s and troca_fun = 2  ORDER BY HORA_ENVIO DESC LIMIT 1',(chat_id,data_atual,))
                                    troca_codfun = cursor.fetchone()
                                    empr_atualizada = troca_codfun[4]
                                    codfun_atualizado = troca_codfun[5]
                                    codigo_atualizado = str(empr_atualizada) + str(codfun_atualizado)
                                    print(f'Código que foi atualizado: {codigo_atualizado}')
                                ...
                                # cursor.execute('Select * From tlg_Bipnotific Where cod_grupo = %s And hora_retorno >= %s And hora_retorno <= %s And Coalesce(TROCA_FUN,0) = (Select Max(Coalesce(TROCA_FUN,0)) From tlg_Bipnotific Limit 1) order by hora_envio asc  Limit 1',(chat_id, data1,data2,))

                                cursor.execute("SELECT * FROM tlg_Bipnotific WHERE cod_grupo = %s AND hora_retorno >= %s AND hora_retorno <= %s AND Coalesce(TROCA_FUN, 0) = (SELECT Max(Coalesce(TROCA_FUN, 0)) FROM tlg_Bipnotific WHERE cod_grupo = %s AND hora_retorno >= %s AND hora_retorno <= %s Limit 1) ORDER BY hora_envio ASC Limit 1",(chat_id,data1, data2, chat_id, data1, data2,))
                                ...
                                if resultado is None:
                                    ...
                                    context.user_data['cod_empr'] = cod_empr
                                    context.user_data['cod_fun'] = cod_fun
                                    context.user_data['funcionario'] = funcionario_codigo[0]

                                   
                                elif resultado is not None:
                                    empr_tabela = resultado[4]
                                    codfun_tabela = resultado[5]
                                    # troca_fun = resultado[11]
                                    troca_fun = resultado[10]
                                    print(f'Troca_fun: {troca_fun}')
                                    print('Condição de excessão!')
                                    codigo_funcionario_tabela = str(empr_tabela) + str(codfun_tabela)
                                    print(codigo_funcionario_tabela)
                                    if codigo == codigo_funcionario_tabela and troca_fun < 1:
                                        ...
                                        resposta = f"Obrigado {funcionario_codigo[0].split()[0]}, recebido!"
                                        await context.bot.send_message(chat_id=int(chat_id),text=resposta)
                                    elif codigo == codigo_atualizado:
                                        cursor.execute('''update tlg_bipnotific set empr = %s, codfun = %s, hora_retorno = %s
                                        where cod_grupo = %s and codfun is null 
                                        and hora_envio = (select hora_envio from tlg_bipnotific 
                                        where cod_grupo = %s order by hora_envio desc limit 1)''',(cod_empr, cod_fun, agora ,chat_id, chat_id,))
                                        database.commit()
                                        print('Retorno registrado!! (condição codigo==codigo_atualizado)')
                                        resposta = f"Obrigado {funcionario_codigo[0].split()[0]}, recebido!"
                                        await context.bot.send_message(chat_id=int(chat_id),text=resposta)
                                    else:
    ...
    if var_temp == 1:
        ...
        if message.isalpha():
            ...
            if message.upper() == 'SIM':
                cursor.execute('''update tlg_bipnotific set empr = %s, codfun = %s, hora_retorno = %s, troca_fun = 0
                where cod_grupo = %s 
                and hora_envio = (select hora_envio from tlg_bipnotific 
                where cod_grupo = %s order by hora_envio desc limit 1)''',(context.user_data['cod_empr'], context.user_data['cod_fun'], agora ,chat_id, chat_id,))
                database.commit()
                print('Retorno registrado!')
                # resposta = f"Obrigado {funcionario_global.split()[0]}, recebido!\n(Se NÃO é você, clique em /trocar)"
                resposta = f"Obrigado {context.user_data['funcionario'].split()[0]}, recebido!\n(Se NÃO é você, clique em /trocar)"
                await context.bot.send_message(chat_id=int(chat_id),text=resposta)
...
async def trocar_fun(update: Update, context: CallbackContext):
    print('Entrando na função de troca de funcionário...')
    chat_id = update.message.chat_id
    chat_name = update.message.chat.title
    await context.bot.send_message(chat_id=chat_id, text='Digite seu código...')
    database = await conexao_banco()
    cursor = database.cursor()
    data_atual = datetime.now().strftime("%Y-%m-%d")
    cursor.execute('SELECT HORA_RETORNO FROM TLG_BIPNOTIFIC WHERE COD_GRUPO = %s AND HORA_ENVIO >= %s  ORDER BY HORA_ENVIO DESC LIMIT 1',(chat_id,data_atual,))
    ultimo_bip_hora_retorno = cursor.fetchone()[0]
    print(f"Última hora retorno (trocar_fun): {ultimo_bip_hora_retorno}")
    cursor.execute('update tlg_Bipnotific set empr = null, codfun = null, hora_retorno = null, troca_fun = 2 where cod_grupo = %s and hora_retorno = %s  ',(chat_id,ultimo_bip_hora_retorno,))
    database.commit()
    print(f'Troca de funcionário realizada no grupo: {chat_name}')
    # await context.bot.send_message(chat_id=chat_id, text='Troca realizada com sucesso! Digite o seu código correto agora...')

async def corrigir_fun(update: Update, context: CallbackContext):
    print('Entrando na função de correção de retorno...')
    global data_inicial, var_temp
    chat_id = update.message.chat_id
    chat_name = update.message.chat.title
    ...
    print(f'Resultado_id: {resultado_id}')
    cursor.execute('update tlg_Bipnotific set empr = null, codfun = null, hora_retorno = null where cod_grupo = %s and hora_retorno >= %s and hora_retorno <= %s and id_notifica = %s',(chat_id, data1,data2,resultado_id,))
    database.commit()
    print(f'Coreeção de retorno realizada no grupo: {chat_name}')
    # await context.bot.send_message(chat_id=chat_id, text='Troca realizada com sucesso! Digite o seu código correto agora...')
