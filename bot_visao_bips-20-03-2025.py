...
async def start_bot(chat_id, chat_name, context):
    ...
    while True:
        ...
        if nova_hora_inicio and nova_hora_fim:
            # Verificar se estamos dentro do intervalo de tempo 20:00:00 até 05:00:00
            # Para resetar a data_inicial para o início da rotina
            if nova_hora_inicio.time() <= agora.time() <= nova_hora_fim.time():
                # Resetar data_inicial para a data de início da rotina
...
async def handle_tips_code(update: Update, context: CallbackContext) -> None:
    global var_temp, cod_empr_global, cod_fun_global, funcionario_global, data_atual
    database = await conexao_banco()
    ...
    if message.replace(" ", "").replace("\n", "").isdigit():
        ...
        lista_empresas = {'406','407','409','410', '412'}
