...
#                 if tentativas == 1:
#                     cursor.execute(
#                         '''INSERT INTO TLG_BIPNOTIFIC (ID_BIP, COD_GRUPO, COD_SUPERVISAO, HORA_ENVIO) 
#                         VALUES (%s, %s, %s, %s)''',
...
#                 cursor.execute(
#                     'SELECT HORA_ENVIO FROM TLG_BIPNOTIFIC WHERE COD_GRUPO = %s ORDER BY HORA_ENVIO DESC LIMIT 1',
#                     (chat_id,)
#                 )
#                 hora_envio_explicita = cursor.fetchone()[0]

#                 cursor.execute(
#                     '''SELECT * FROM TLG_BIPNOTIFIC WHERE COD_GRUPO = %s ORDER BY HORA_ENVIO DESC LIMIT 1''',
#                     (chat_id,)
#                 )
...
#                 cursor.execute(
#                     'select codfun from tlg_Bipnotific where cod_grupo = %s and hora_retorno >= %s and hora_retorno <= %s order by hora_envio asc',
#                     (chat_id, data1, data2)
#                 )
...
#                     cursor.execute(
#                         'SELECT * FROM TLG_BIPNOTIFIC WHERE COD_GRUPO = %s AND HORA_ENVIO >= %s AND HORA_RETORNO IS NOT NULL ORDER BY HORA_RETORNO DESC LIMIT 1',
#                         (chat_id, data_atual)
#                     )
...
#                     else:
#                         print("Última resposta não encontrada")
#                 else:
...
#             novos_horarios = await gerar_horarios(
#                 active_chats[chat_id]["ultima_hora_inicio"],
#                 active_chats[chat_id]["ultima_hora_fim"],
#                 chat_name
#             )
...
#                     database.commit()
#                     print(f'Resultado proc sim/não: {resultado}')
#                     resultado_obg = resultado[0].split()[0]
#                     if resultado_obg == "Obrigado" :
#                         nome_fun = resultado[0].split()[1].replace(",","")
#                         await context.bot.send_message(chat_id=chat_id, text=f"{nome_fun}✅")
