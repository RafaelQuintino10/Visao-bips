...
async def handle_tips_code(update: Update, context: CallbackContext) -> None:
    ...
    if message.replace(" ", "").replace("\n", "").isdigit():
        ...
        if message[:3] in lista_empresas and len(message)<6 or message[:3] not in lista_empresas:
            print('Condição código inválido!')
            await context.bot.send_message(chat_id=int(chat_id), text='Código Inválido! Tente novamente...')

        elif message[:3] in lista_empresas and len(message)>=6:
            ...
            try:
                ...
                #                     codigofuncionario_posto = codigo
                #                     nomefuncionario_posto = funcionario_codigo[0].split()[0]
                #                     reposta_valida = True
                #                 elif codigofuncionario_posto != codigo:
                ...
                #                     database.commit()
                #                     # cursor.execute('SELECT NAME FROM PUBLIC.USER WHERE CODE =%s', (str(codigo),))
